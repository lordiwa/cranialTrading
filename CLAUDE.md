...
Full current `@smoke` set (verified 2026-08-08, updated 2026-08-21 — TASK-259/TASK-265 removed `matches/match-management.spec.ts` "matches page loads with tab navigation": mounting that view runs `cleanExpiredMatches()`, which deletes expired match documents via `writeBatch` — NOT read-only, see TASK-265): `collection/collection-crud.spec.ts` "collection page loads with card grid visible", `user-profile/user-profile.spec.ts` "view public user profile", `auth/login.spec.ts` "successful login redirects to the /inicio landing", `smoke/navigation.spec.ts` "nav: Collection link navigates to /collection" and "tab switching renders correct content", `search/search.spec.ts` "search by card name returns results grid", `messages/messages.spec.ts` "messages page loads conversation list". That's 7 tests, tagged in their title (e.g. `test('successful login redirects to the /inicio landing @smoke', ...)`), run across whatever project(s) Playwright routes them to (`chromium` / `no-auth-tests`) — no separate config needed. Together they cover: auth (login), and page-load/navigation for collection, messages, search, user-profile, and tab switching — enough to prove the production-configured bundle boots and can authenticate, without creating a single document. Matches page-load coverage against production is gone until TASK-265 lands a read-only replacement.
