...
- **Mutators — real cleanup, not just exclusion:** `deck-crud.spec.ts` "create new deck" and `binders.spec.ts` "create new binder" now delete their own fixture at the end of the same test (reusing the same delete flow their sibling "delete" test in the file already exercises) — previously they left the deck/binder behind on every run, in the push-to-develop full suite too, not just nightly. `collection-crud.spec.ts` "add card" cleans up too, but via net-zero doc count rather than literal-fixture deletion: the shared CI collection already has 25+ pre-existing "Lightning Bolt" entries (different printings) and card_index writes are eventually consistent (optimistic patch + deferred refresh — the "card_index persist race pattern"), so no UI signal right after adding reliably identifies the exact doc just created — a name-filtered search can't tell it apart from the pre-existing duplicates either. It deletes whatever card is at grid index 0 instead (the same mechanics the "delete card" test below uses), which keeps the doc count bounded — the collection composition rotates by one card per run rather than growing without bound. `auth/register.spec.ts` "successful registration" is the one true exception: it creates a real Firebase Auth account + Firestore user doc with no UI-exposed deletion path at all, so it's excluded from nightly via a `@nightly-skip` tag (`--grep-invert @nightly-skip`, wired as `npm run e2e:nightly`) instead of cleaned up. The rest of `register.spec.ts` (validation/duplicate checks that don't create accounts) still runs nightly.
