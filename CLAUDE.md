...
Full current `@smoke` set (verified 2026-08-08, corrected 2026-08-21 by re-deriving from the code via `grep -rn "test(.*@smoke" e2e/specs/ | wc -l` rather than trusting this line — see the correction note below): `collection/collection-crud.spec.ts` "collection page loads with card grid visible", `auth/login.spec.ts` "successful login redirects to the /inicio landing", `smoke/navigation.spec.ts` "nav: Collection link navigates to /collection" and "tab switching renders correct content", `search/search.spec.ts` "search by card name returns results grid", `messages/messages.spec.ts` "messages page loads conversation list". That's **6 tests**, tagged in their title (e.g. `test('successful login redirects to the /inicio landing @smoke', ...)`), run across whatever project(s) Playwright routes them to (`chromium` / `no-auth-tests`) — no separate config needed. Together they cover: auth (login), and page-load/navigation for collection, messages, search, and tab switching — enough to prove the production-configured bundle boots and can authenticate, without creating a single document.

Two pieces of page-load coverage against production are currently MISSING from this set, both worth naming so nobody re-derives a wrong "why" later:
- **Matches**: `matches/match-management.spec.ts` "matches page loads with tab navigation" lost its tag in TASK-259/TASK-265 (2026-08-21) — mounting that view runs `cleanExpiredMatches()`, which deletes expired match documents via `writeBatch`, so it was never read-only. Tracked in TASK-265.
- **User profile**: `user-profile/user-profile.spec.ts` "view public user profile" lost its `@smoke` tag as an unnoticed side effect of `f6b15b2` (TASK-256, 2026-08-18) — the fix touched the test but the tag silently disappeared, and this doc line kept claiming it was still tagged for 3 days (2026-08-18 → 2026-08-21) before anyone measured otherwise. Restoring the tag is deliberately NOT done here: it needs (a) confirmation the test is genuinely read-only (TASK-263 AC3) and (b) survives the planned prod-account wipe (TASK-267) without going red on missing profile data. Left for TASK-263's full audit.

**Correction note (2026-08-21):** this line's "8 tests" claim was wrong for at least 3 days before anyone caught it — both the matches loss (introduced the same day it was caught) and the user-profile loss (from 2026-08-18, uncaught for 3 days) slipped through because the number was cited from this doc, not re-measured from the code. Same failure mode as an empty-green test, just in documentation instead of a test file: a stated number stood in for a measurement. Treat any test-count claim in this file as a starting hypothesis to verify with `grep`, not as ground truth — TASK-263 AC1 re-derives this set from the code rather than starting from this paragraph.
...
#### Recreating/repairing dev's test data (`qa_mtg`) — TASK-290

The nightly sensor (`user-profile.spec.ts`'s "browse public cards on profile with text search filter", floor of 1000 — never touch that floor or that test) reads `users/{uid}/public_card_index`, which is DERIVED from the root `public_cards` collection. Two collections can drift out of parity independently of each other and of `users/{uid}/cards` — MEASURED 2026-09-16: `users/{uid}/cards` had 2,202 documents with status=sale AND public=true, but the root `public_cards` collection (userId==uid) had only 400, and its derived index matched that same wrong 400 — a real drift, not a broken sensor.

**Which account, and why that literal matters.** `qa_mtg` (email `qa@cranialtrading.com`, uid `90PkdmyFKrVm1RLDXjInJdlYXy73`) is dev's ONLY account (TASK-267 wipe) and the one the whole E2E suite authenticates as (`TEST_USER_A_*` in `.env.local`, `e2e/auth.setup.ts`). The username literal `qa_mtg` is hardcoded in `user-profile.spec.ts` (`PUBLIC_PROFILE_USERNAME` / `OWN_PROFILE_USERNAME`) — recreating the account under any other username breaks that spec outright. **Do not touch or delete `/usernames/qa`** — a second, deliberately orphaned index entry pointing at the same uid, kept as live evidence for TASK-268 (see that spec file's header comment).

**Procedure, in order:**

1. If the account itself is gone (not just its public data — check first with `artifacts/scratch-290-ac1.mjs`, which reads `/usernames/qa_mtg`, `/users/{uid}`, `users/{uid}/cards`, root `public_cards`, `users/{uid}/public_card_index`, and Auth, all by direct count, never by assuming a doc's presence means it's populated): re-seed cards with `node scripts/seed-e2e-bulk-cards.mjs` (signs in as the real account, imports real cards via the same Cloud Function the app's CSV import uses, then rebuilds `card_index`). That script does not touch `public_cards` or the public index at all — see its own header for why (avoiding a hand-written index is this project's central `card_index` bug family).

   **This step has a dead end, found at review (TASK-290 round 2, MEDIUM-3): `seed-e2e-bulk-cards.mjs` calls `signInWithEmailAndPassword` (its own line 168) — it requires the Auth account to already exist and creates nothing. It is NOT a fix for "the account itself is gone." That is precisely the scenario this whole procedure exists for (dev wiped, TASK-267), so this step is currently a dead end for its own stated trigger, not scripted anywhere in this repo.**

   Recreating the account by hand needs all three of the following (derived by reading `src/stores/auth.ts`'s registration/self-heal path, `reserveUniqueUsername` around line 761, and the `/users/{uid}` write around line 379 — **NOT VERIFIED by actually running them against dev**, so confirm each write lands and re-measure with `artifacts/scratch-290-ac1.mjs` before trusting the account is usable):
   - Create the Firebase Auth user for `qa@cranialtrading.com` (Firebase console or `firebase-admin`'s `createUser`), uid `90PkdmyFKrVm1RLDXjInJdlYXy73`.
   - Create `/users/{uid}` with at least `{ username: 'qa_mtg', location: '', createdAt: <timestamp> }` (matches the shape `stores/auth.ts` writes on self-heal).
   - Create the reservation `/usernames/qa_mtg` as `{ uid: '90PkdmyFKrVm1RLDXjInJdlYXy73', createdAt: <timestamp> }` (matches `reserveUniqueUsername`'s write shape) — **the literal `qa_mtg` matters**: it is hardcoded in `user-profile.spec.ts` (`PUBLIC_PROFILE_USERNAME` / `OWN_PROFILE_USERNAME`), and any other username breaks that spec outright.
   - Only then does step 1's `seed-e2e-bulk-cards.mjs` have an account to sign into.
2. Bring the root `public_cards` collection back to parity with `users/{uid}/cards` (status=sale/trade AND public=true): `node scripts/repair-public-cards-parity.mjs --uid=90PkdmyFKrVm1RLDXjInJdlYXy73 [--dry-run]`. Reuses `buildPublicCardDoc` (`src/services/publicCards.ts`, the single source of truth for that document's shape) via `scripts/lib/loadBuildPublicCardDoc.mjs` rather than a hand-copied literal — see that loader's header for why a plain static import isn't possible (it would eagerly boot the browser-only Firebase Auth client). Upserts every eligible card's document (self-healing content drift, not just filling gaps) and deletes any document for the seller that is no longer eligible. Idempotent — MEASURED 2026-09-16: running it twice in a row left `public_cards` at 2,202 both times, with the second run's diff reporting zero to-create.
3. Rebuild the index from the now-correct `public_cards`: `node scripts/reconcile-public-card-index.mjs --uid=90PkdmyFKrVm1RLDXjInJdlYXy73`. Growing an index (400 → 2,202, this incident's actual direction) does not trip `requiresCollapseConfirmation` (`functions/lib/publicCardIndexExecutor.js`) — verified by running it, not assumed: it printed `strategy=rebuild-by-id`, never `refused`. A THIRD run of this same command against an already-consistent index printed `strategy=noop` — confirms the reconciler itself is idempotent too.
4. Verify AC2/AC3 by direct count (never by "the test passed"): `node artifacts/scratch-290-ac1.mjs` — confirm `users/{uid}/cards` public=true count, root `public_cards` (userId==uid) count, and `public_card_index` entries/`_meta.count` all match. TASK-290's own repair left all four at 2,202.

Both `repair-public-cards-parity.mjs` and `reconcile-public-card-index.mjs` default to `cranial-trading-dev` and refuse any other `--project` without `--i-know-what-im-doing` — the same rail every script in `scripts/` that can mutate Firestore uses.
