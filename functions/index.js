...
// TASK-247: the public-profile index. functions/lib/publicCardIndex.js and
// functions/lib/publicCardIndexExecutor.js are the pure assembly/diagnosis/
// planning/batching layers (tandas 1/2a/2b) — see their own headers for the
// full contracts. The actual Firestore orchestration that consumes them
// lives in publicCardIndexReconciler.js (review round 2, MEDIUM-4: shared
// verbatim with scripts/reconcile-public-card-index.mjs, so there is
...
//
// Thin wiring around functions/lib/publicCardIndexReconciler.js's
// `reconcilePublicCardIndexForUser` — the actual orchestration (Firestore
// reads/writes designed in this tanda) lives there now, shared verbatim
// with scripts/reconcile-public-card-index.mjs (review round 2, MEDIUM-4:
// this used to be ~150 duplicated lines between the two files, which is
// exactly how the collection-path HIGH bug earlier in this tanda had to be
// fixed twice). See that module's header for STORAGE LAYOUT and the full
// mechanism write-up (ordering/atomicity, the lease, the collapse guard,
// the malformed-chunk-id/path-mismatch handling).
//
// AUTHORIZATION (ticket-mandated write-up, unchanged by the refactor).
// This function is SELF-ONLY: `targetUserId = request.auth.uid`, exactly
// the pattern buildCardIndex and queryCardIndex already use, and there is
// no client-suppliable userId parameter anywhere in request.data for this
// function to trust — TASK-214 closed exactly that hole for queryCardIndex
// (a client-supplied userId let any logged-in caller read another user's
// card_index, which holds the WHOLE inventory, not just what's public) and
// TASK-211 closed the same shape for buildCardIndex. Reintroducing an
// optional targetUserId here, even scoped to public_cards only, would be
// the third occurrence of that exact family.
//
// The ticket also asks for an administrative path that CAN target an
// arbitrary seller. This project has no admin-claim / role infrastructure
// (grepped: no isAdmin, no custom claims, anywhere in functions/ or src/),
// and building one is out of this tanda's closed scope. The administrative
// path is scripts/reconcile-public-card-index.mjs instead — its OWN admin
// SDK credentials (`gcloud auth application-default login`), same pattern
// as scripts/card-index-fixture.mjs and the nightly E2E admin teardown
// (e2e/helpers/admin.ts, GOOGLE_APPLICATION_CREDENTIALS). It requires
// publicCardIndexReconciler.js directly — the SAME function this onCall
// wraps — never calls this callable, and never needs request.auth at all,
// so no arbitrary-userId path is ever exposed over HTTPS.
// ============================================================
...
exports.reconcilePublicCardIndex = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      // LOW fix (review round 2): the previous version here re-threw on
      // `error.code` truthy, which is ALSO true for a raw Firestore SDK
      // error (its numeric gRPC status lives at `.code` too) — a real,
      // unexpected failure would silently skip the logger.error below and
      // vanish from the logs. Check the actual HttpsError class instead;
      // this is new code in this tanda, not the pre-existing pattern
      // elsewhere in this file that copied the loose check (queryCardIndex,
      // buildCardIndex) — out of scope to change there.
