...
<template>
  <Teleport to="body">
    <div role="status" aria-live="polite" aria-atomic="false" class="fixed bottom-4 right-4 z-[100] flex flex-col gap-2 max-w-sm w-full px-4 md:px-0">
      <TransitionGroup name="toast">
        <div
            v-for="toast in toastStore.toasts"
            :key="toast.id"
            :class="[
              'bg-primary border shadow-strong rounded-lg',
        ...
        >
          ...
          <div class="px-md py-md flex items-center gap-2">
            ...
            <!-- Success icon -->
            <svg aria-hidden="true" v-else-if="toast.type === 'success' || (isProgressToast(toast) && toast.status === 'complete')" class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
            </svg>
            <!-- Error icon -->
            <svg aria-hidden="true" v-else-if="toast.type === 'error' || (isProgressToast(toast) && toast.status === 'error')" class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
            </svg>
            <!-- Info icon -->
            <svg aria-hidden="true" v-else-if="toast.type === 'info'" class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div
              ...
              class="h-1 bg-surface-2 overflow-hidden"
          >
            <div
